...
class Up(nn.Module):
    ...
    def forward(self, x1, x2, attn_weight):
        ...
        # x2 encode embed: [bs, d_in, T, H, W] H =2h
        # attn_weight [n_head, bs, 16, 16, T, T]
        ...
        # v_reshaped [n_head*bs*16*16, T, d_in] 
        ...
        attn_weight = attn_weight.permute(0, 1, 4, 5, 2, 3).contiguous()        # [n_head, bs, T, T, 16, 16]
        attn_weight = attn_weight.view(self.n_head*bs, T*T, h, w)               # [n_head*bs, T*T, 16, 16]
        attn_weight = self.up_attn(attn_weight)
        attn_weight = attn_weight.permute(0, 2, 3, 1).contiguous().view(-1, T, T)  # [n_head*bs*h*w, T, T]
...
class unet(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.inc(x)                                                # x [bs, 3, 10, 128, 128]  x1 [bs, 64, 10, 128, 128]
        x2 = self.down1(x1)                                             # x2 [bs, 64, 10, 64, 64]
        x3 = self.down2(x2)                                             # x3 [bs, 64, 10, 32, 32]
        x4 = self.down3(x3)                                             # x4 [bs, 128, 10, 16, 16]
        bs, c, T, x4_h, x4_w = x4.shape
        
        attn_x4, attn_weight = self.attention_heads(x4, x4, x4)         # attn_x4 [n_head, bs, 16, 16, T, d_in]   # attn_weight [n_head, bs, 16, 16, T, T]
        
        # concatenate heads
        attn_x4 = attn_x4.permute(1, 4, 2, 3, 5, 0).contiguous().view(bs, T, x4_h, x4_w, -1) # attn_x4 [bs, T, h, w, d_in*n_head]
        ...
        attn_x4 = self.mlp2(attn_x4).permute(0, 4, 1, 2, 3)             # put C to 1st dim
        attn_x4 = self.BatchNorm3d2(attn_x4).permute(0, 2, 3, 4, 1)     # put C to 3rd dim
        attn_x4 = self.ReLU(attn_x4)
        
        attn_x4 = self.dropout(attn_x4)
        attn_x4 = self.outlayernorm(attn_x4).permute(0, 4, 1, 2, 3)     # put C to 1st dim, attn_x4 [bs, 128, 10, 16, 16]
        
        # skip connection: concatenate n_heads * encoder embeds to the output
        # too large channel dimension.
        x = self.up1(attn_x4, x3, attn_weight)                          # up1 x [bs, 128, 10, 32, 32]
        x = self.up2(x, x2, attn_weight)                                # up2 x [bs, 64, 10, 64, 64]
        x = self.up3(x, x1, attn_weight)                                # up3 x [bs, 64, 10, 128, 128]
